...
def train_ddpg_agent(model_env: CSTRPITuningEnv, total_timesteps: int = 25000) -> DDPG:
    ...
    model = DDPG("MlpPolicy", vec_env, action_noise=action_noise, verbose=0, learning_rate=1e-3, buffer_size=50000, batch_size=128, gamma=0.98, tau=0.01)
    model.learn(total_timesteps=total_timesteps, progress_bar=False)
    return model

def train_sac_agent(model_env: CSTRPITuningEnv, total_timesteps: int = 25000) -> SAC:
    vec_env = DummyVecEnv([lambda: CSTRPITuningEnv(model_env.model, model_env.config)])
    model = SAC("MlpPolicy", vec_env, verbose=0, learning_rate=1e-3, buffer_size=50000, batch_size=128, gamma=0.98, tau=0.01, ent_coef='auto')
    model.learn(total_timesteps=total_timesteps, progress_bar=False)
    return model